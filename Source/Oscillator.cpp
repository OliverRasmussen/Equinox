...
double Oscillator::sine(double frequency)
{
    ...
    return nextOutputSample(frequency);
}
...
double Oscillator::nextOutputSample(double frequency)
{
    double output = 0.0f;
    switch (currentWaveform)
    {
        case sineWave:
            output = sineTable.lower_bound(frequency)->second[(int)phase];
            break;
            
        case sawWave:
            output = sawTable.lower_bound(frequency)->second[(int)phase];
            break;
            
        case squareWave:
            output = squareTable.lower_bound(frequency)->second[(int)phase];
            break;
            
        case triangleWave:
            output = triangleTable.lower_bound(frequency)->second[(int)phase];
            break;
    }
    float fractionFrequency = frequency * (float)waveTableSize / (float)sampleRate;
    phase = fmod(phase + fractionFrequency, waveTableSize);
    return output;
}
...
void Oscillator::generateWavetable(waveform waveform)
{
    ...
    for (int i = 0; i < frequencyRange; i++)
    {
        ...
        while (harmonicFrequency < nyquistFrequency)
            {
                // Delta of the angle
                double angleDelta = M_2PI / (waveTableSize - 1) * harmonic;
                ...
                for (int i = 0; i < waveTableSize; ++i)
                {
                    ...
                    double sample = waveform == triangleWave ? cos(currentAngle) / (harmonic * harmonic) : sin(currentAngle) / harmonic;
