...
double WavetableOscillator::square(double frequency)
{
    return nextOutputSample(squareTable, frequency);
}

double WavetableOscillator::saw(double frequency)
{
    return nextOutputSample(sawTable, frequency);
}

double WavetableOscillator::triangle(double frequency)
{
    return nextOutputSample(triangleTable, frequency);
}

double WavetableOscillator::noise() const
{
    return (rand() / (float)RAND_MAX)*2-1;
}

double WavetableOscillator::nextOutputSample(std::map<double, std::array<double, waveTableSize>>& wavetable, double frequency)
{
