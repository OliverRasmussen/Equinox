...
void WavetableOscillator::setTableIterator()
{
    ...
    {
        // Find iterator to a frequency table corresponding to the current frequency,
        // or closest match
        auto it = currentTable->lower_bound(currentFrequency);

        // Set tableIterator to reference the given frequency table,
        // or last frequency table if iterator is out of currentTable range
...
void WavetableOscillator::generateWavetable(Waveform waveform)
{
    ...
    for (int i = 0; i < frequencyRange; i++)
    {
        ...
        while (harmonicFrequency < nyquistFrequency)
            {
                // Delta of the angle
                double angleDelta = M_2PI / waveTableSize  * (float)harmonic;
                ...
                for (int i = 0; i < waveTableSize; ++i)
                {
                    ...
                    double sample = waveform == Waveform::triangleWave ? sin(currentAngle + M_PI/2) / (float)(harmonic * harmonic) : sin(currentAngle) / (float)harmonic;
